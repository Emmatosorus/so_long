...
/*   Updated: 2024/02/13 14:30:02 by epolitze         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_xpm(t_main *main, char *path, int pos)
{
	...
	if (!main->var.xpm[pos])
		xpm_error_exit(pos, main, "Malloc has failed : inits.c : 46\n");
	...
	if (!main->var.xpm[pos]->data.addr)
		xpm_error_exit(pos, main, "MLX has failed us once more : inits.c : 59\n");
}
...
static void	xpm_inits(t_main *main)
{
	...
	if (!main->var.xpm)
		error_exit(main, "Malloc has failed : inits.c : 70\n");
	...
	make_xpm(main, "sprites/world/forest.xpm", 12);
	make_xpm(main, "sprites/world/forest_up.xpm", 13);
	make_xpm(main, "sprites/world/forest_down.xpm", 14);
	make_xpm(main, "sprites/world/forest_right.xpm", 15);
	make_xpm(main, "sprites/world/forest_left.xpm", 16);
	make_xpm(main, "sprites/assets/move_background.xpm", 17);
}
